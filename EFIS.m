function [Output,SYST]=EFIS(Input,SYST,Mode)
threshold=exp(-1/4);
omega=100;
if strcmp(Mode,'Learning')==1
    if isempty(SYST)==1
        [SYST.MN,SYST.A,SYST.CovMat,SYST.GM,SYST.GX,SYST.C,SYST.P,SYST.LX,SYST.S,SYST.SumL,SYST.Ind,SYST.CarD,Output.Ye]=...
            LearningALM(Input.datain,Input.dataout,omega,threshold,Input.FF);
    else
        [SYST.MN,SYST.A,SYST.CovMat,SYST.GM,SYST.GX,SYST.C,SYST.P,SYST.LX,SYST.S,SYST.SumL,SYST.Ind,SYST.CarD,Output.Ye]=...
...
function [ModelNumber,A,C,Global_mean,Global_X,center,prototype,Local_X,Support,sum_lambda,Index,L,Ye]=LearningALM(data0,Y0,omega,threshold,FF)
CL=size(Y0,2);
[L,W]=size(data0);
center=data0(1,:);
prototype=data0(1,:);
...
for ii=2:1:L
    [ModelNumber,A,C,Global_mean,Global_X,center,prototype,Local_X,Support,sum_lambda,Index,Ye(ii,:)]=...
        SystemUpdating(ModelNumber,A,C,Global_mean,Global_X,center,prototype,Local_X,Support,sum_lambda,Index,data0(ii,:),Y0(ii,:),ii,omega,threshold,CL,W,FF);
end
end
function [ModelNumber,A,C,Global_mean,Global_X,center,prototype,Local_X,Support,sum_lambda,Index,Ye]=...
    SystemUpdating(ModelNumber,A,C,Global_mean,Global_X,center,prototype,Local_X,Support,sum_lambda,Index,datain,yin,ii,omega,threshold,CL,W,FF)
